//! The game cartridge: the ROM image plus an optional memory bank controller (MBC).
//!
//! Two cases are handled. A *NoMBC* cart (Tetris and other 32 KiB ROM-only games)
...
///
/// Every field follows the same pattern: read a byte (or slice) at a fixed offset and
/// interpret it. The four below — mapper, title, ROM size, RAM size — each decode one
/// such offset; Pan Docs' "The Cartridge Header" has the full encodings. Sanity check
/// against Tetris's known header: title "TETRIS", ROM 2 banks (32 KiB), RAM 0 bytes.
fn parse_header(rom: &[u8]) -> Header {
    // --- cartridge type @ 0x0147 ------------------------------------------------
    // One byte selects the mapper hardware. We decode a friendly name here; the
    // cartridge acts on the type (NoMBC vs minimal MBC1) in its `read`/`write`.
    ...
    // --- title @ 0x0134..=0x0143 ------------------------------------------------
    // 16 bytes of ASCII, zero-padded. We decode the slice and strip the trailing
    // 0x00s with `trim_end_matches('\0')`; `from_utf8_lossy` stays safe on odd bytes.
    ...
    // --- RAM size @ 0x0149 ------------------------------------------------------
    // Another code -> a byte count, mapped below:
    //   0x00 -> 0,  0x02 -> 8 KiB,  0x03 -> 32 KiB,  0x04 -> 128 KiB,  0x05 -> 64 KiB.
    // (Tetris is 0x00 -> no RAM.)
